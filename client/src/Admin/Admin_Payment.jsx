...
const Admin_Payment = () => {
  ...
  const segments = [{ value: 20 }, { value: 40 }, { value: 60 }, { value: 80 }, { value: 100 }];
  ...
  const handleExportInvoice = () => {
    ...
    const headers = ["Invoice ID", "Invoice Name", "Invoice Date", "Invoice Amount", "Payment Method", "Status"];
    const csvRows = [
      headers.join(","),
    ...
    link.setAttribute("download", `donation_invoice_${new Date().toISOString().split("T")[0]}.csv`);
  ...
  // Download PDF with invoice details
  const handleDownloadPDF = () => {
    if (!selectedPayment) {
      toast.error("Please select a payment to download invoice");
      return;
    }
    ...
    const invoiceHTML = `
      <div style="padding: 40px; font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto;">
        ...
        <div style="text-align: center; margin-bottom: 30px;">
          <h1 style="color: #10b981; font-size: 32px; margin: 0;">INVOICE</h1>
        </div>
        
        <!-- Invoice Details -->
        <div style="margin-bottom: 20px;">
          <div style="display: flex; align-items: center; margin-bottom: 10px;">
            <strong>Invoice ID:</strong> <span style="margin-left: 10px;">${selectedPayment.transactionId?.slice(-6) || "N/A"}</span>
          </div>
          <div style="margin-bottom: 10px;">
            <strong>Date:</strong> <span style="margin-left: 10px;">${selectedPayment.createdAt ? new Date(selectedPayment.createdAt).toLocaleDateString() : "N/A"}</span>
          </div>
          <div>
            <strong>Status:</strong> <span style="margin-left: 10px; padding: 4px 12px; background-color: ${selectedPayment.status === "Successful" ? "#d1fae5" : "#fee2e2"}; color: ${selectedPayment.status === "Successful" ? "#065f46" : "#991b1b"}; border-radius: 4px; font-size: 12px;">${selectedPayment.status || "N/A"}</span>
          </div>
        </div>
        ...
        <div style="margin-bottom: 25px;">
          <h3 style="color: #1f2937; font-size: 16px; margin-bottom: 10px;">Billing Address</h3>
          <p style="color: #6b7280; font-size: 14px; line-height: 1.6;">${selectedPayment.billingAddress || "No address provided"}</p>
        </div>
        
        <!-- Payment Details -->
        <div style="margin-bottom: 25px;">
          <h3 style="color: #1f2937; font-size: 16px; margin-bottom: 15px;">Payment Details</h3>
          <table style="width: 100%; border-collapse: collapse;">
            <thead>
              <tr style="background-color: #10b981;">
                <th style="padding: 12px; text-align: left; color: white; border: 1px solid #10b981;">Description</th>
                <th style="padding: 12px; text-align: left; color: white; border: 1px solid #10b981;">Details</th>
              </tr>
            </thead>
            <tbody>
              <tr style="background-color: #f9fafb;">
                <td style="padding: 10px; border: 1px solid #e5e7eb;">Bill Name</td>
                <td style="padding: 10px; border: 1px solid #e5e7eb;">${selectedPayment.user || "N/A"}</td>
              </tr>
              <tr>
                <td style="padding: 10px; border: 1px solid #e5e7eb;">Payment Method</td>
                <td style="padding: 10px; border: 1px solid #e5e7eb;">${selectedPayment.paymentMethod || "N/A"}</td>
              </tr>
              <tr style="background-color: #f9fafb;">
                <td style="padding: 10px; border: 1px solid #e5e7eb; font-weight: bold;">Amount</td>
                <td style="padding: 10px; border: 1px solid #e5e7eb; font-weight: bold; color: #10b981; font-size: 16px;">${selectedPayment.currency} ${selectedPayment.amount}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div style="margin-top: 30px; padding: 15px; background-color: #f0fdf4; border-left: 4px solid #10b981;">
          <h4 style="color: #065f46; margin-top: 0;">Note</h4>
          <p style="color: #166534; font-size: 13px; line-height: 1.6; margin: 0;">
            Your generous contribution helps us continue our mission of making a meaningful impact. 
            Every donation, no matter the size, plays a vital role in supporting our cause. 
            Thank you for your commitment to bringing about positive change.
          </p>
        </div>
        
        <!-- Footer -->
        <div style="margin-top: 40px; text-align: center; color: #9ca3af; font-size: 12px; border-top: 1px solid #e5e7eb; padding-top: 20px;">
          <p style="margin: 0;">Guardian Earth - Natural Disaster Management System</p>
          <p style="margin: 5px 0 0 0;">Thank you for your support!</p>
        </div>
      </div>
    `;
            ...
            <div className="flex flex-col bg-white p-5 w-full gap-2 rounded-[5px]">
              <div className="flex flex-row justify-between w-full ">
                <div className="flex flex-col items-start">
                  <h2 className="text-text-primary text-[15px] font-semibold">Donation Income</h2>
                  <p className="text-text-secondary text-[13px] opacity-70  font-normal">Listed below are all conclusion from donation income</p>
                </div>
                <button
                  type="button"
              ...
              <div className="flex flex-row gap-3 mt-2">
                ...
                <div className="border-[1px] h-[250px] w-[35%] rounded-[5px] p-3">
                  <div className="flex flex-col">
                    ...
                    <div className="flex flex-row justify-between">
                      <span className="text-[13px] text-left font-semibold text-gray-400">Total Paid</span>
                      <span className="text-[13px] text-left font-medium text-text-secondary">152</span>
                    </div>
                    <div className="flex flex-row justify-between my-2">
                      <span className="text-[13px] text-left font-semibold text-gray-400">Total Issued</span>
                      <span className="text-[13px] text-left font-medium text-text-secondary ">32</span>
                    </div>
                    <ProgressLinearChart segments={segments} completed={progress} />

                    <div className="flex flex-row justify-around">
                      <div className="flex flex-col justify-center items-center">
                        <div className="w-[1px] h-4 bg-border-default" />
                        <span className="text-[12px] text-left font-normal text-gray-400">Payment Completed</span>
                      </div>
                      <div className="flex flex-col justify-center items-center">
                        <div className="w-[1px] h-4 bg-border-default" />
                        <span className="text-[12px] text-left font-normal text-gray-400">Payment pending</span>
                      </div>
                    </div>
                    <a className="text-[13px] flex flex-row items-center text-left font-semibold text-primary-light ml-1 my-4 mt-5" href="">
                      View details
          ...
          <div className="flex flex-col max-w-screen-lg  w-full  bg-white shadow-sm rounded-[5px] p-6 min-w-[300px]">
            ...
            <div className="flex flex-col">
              <div className="flex flex-row items-center">
                <img src={invoice_jpg} alt="Invoice Image" className="flex w-10 h-10 -ml-2 mr-2" />
                <div className="flex flex-col gap-1">
                  <h3 className="text-[14px] font-medium  w-auto text-left  text-gray-900 ">Invoice</h3>
                  <p className="text-[12px] font-normal  w-auto text-left  text-gray-500 opacity-70">
                    {selectedPayment?.transactionId?.slice(-6) || "N/A"}
                  </p>
                </div>
              </div>
              <div className="flex flex-col gap-1 mt-2">
                ...
                <p className="text-[12px] font-normal w-auto text-left  text-gray-500 opacity-70">Status: {selectedPayment?.status || "N/A"}</p>
              </div>
            </div>
            {/* Seperator */}
            <div className="h-[0.5px] bg-border-default w-full opacity-30 my-2" />
            <div className="flex flex-col my-2">
              <h2 className="text-[14px] font-medium  w-auto text-left  text-gray-900 ">Billing Address</h2>
              <p className="text-[12px] font-normal w-auto text-left mt-2  text-gray-500 opacity-70">
                {selectedPayment?.billingAddress || "No address provided"}
              </p>
            </div>
            ...
            <h2 className="text-[14px] font-medium  w-auto text-left  text-gray-900 ">Payment details</h2>
            <div className="grid grid-rows-3 gap-y-4 mt-4 mx-2 rounded-[4px] border-[1px] border-gray-200 ">
              <div className="flex flex-col justify-start border-b-[1px] border-gray-200 px-2 h-[60px]">
                <p className="text-[13px] font-normal w-auto text-left mt-1 text-gray-500 opacity-70 ">Bill Name</p>
                <p className="text-[13px] font-medium w-auto text-left text-gray-900">{selectedPayment?.user || "N/A"}</p>
              </div>
              <div className="flex flex-col justify-start border-b-[1px] border-gray-200  px-2  h-[60px]">
                <p className="text-[13px] font-normal w-auto text-left mt-1  text-gray-500 opacity-70">Payment Method</p>
                <p className="text-[13px] font-medium w-auto text-left text-gray-900">{selectedPayment?.paymentMethod || "N/A"}</p>
              </div>
              <div className="flex flex-col justify-start pb-2 px-2  h-[60px] ">
                <p className="text-[13px] font-normal w-auto text-left mt-1 text-gray-500 opacity-70">Amount</p>
                <p className="text-[13px] font-medium w-auto text-left text-gray-900">
                  {selectedPayment ? `${selectedPayment.currency} ${selectedPayment.amount}` : "N/A"}
                </p>
              </div>
            </div>

            <h2 className="text-[14px] font-medium  w-auto text-left  text-gray-900 mt-6 mb-2">Note</h2>
            <p className="text-[13px] font-normal w-auto text-left  text-gray-500 opacity-70">
              Your generous contribution helps us continue our mission of making a meaningful impact. Every donation, no matter the size, plays a
              vital role in supporting our cause. Thank you for your commitment to bringing about positive change. Together, we can make a lasting
              difference in the lives of those who need it most.
            </p>

            <h2 className="text-[14px] font-medium  w-auto text-left  text-gray-900 mt-6 mb-2">Files</h2>
            <button
